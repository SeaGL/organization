...
def session_to_html(session):
    video_path = Path(session["file"]).relative_to("uploads")

    collection_url = f'https://archive.org/details/{text_to_url(session["collection"])}'
    creators_html = ", ".join([creator_to_html(session[k]) for k in creator_keys if session[k]])
...
args.index_path.write_text(template.substitute(sessions="\n".join(map(session_to_html, sessions))))
